...
public class FaceCompareActivity extends BaseActivity {
    ...
    public void processImage(Bitmap bitmap, int type) {
        ...
        if ((ageCode | genderCode) != ErrorInfo.MOK) {
            showToast("at lease one of age、gender detect failed! codes are: " + ageCode
                    + " ," + genderCode);
            return;
        }
        ...
        if (!faceInfoList.isEmpty()) {
            if (type == TYPE_MAIN) {
                ...
                int res = mainFaceEngine.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24, faceInfoList.get(0), mainFeature);
                ...
                int res = mainFaceEngine.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24, faceInfoList.get(0), faceFeature);
                if (res == 0) {
