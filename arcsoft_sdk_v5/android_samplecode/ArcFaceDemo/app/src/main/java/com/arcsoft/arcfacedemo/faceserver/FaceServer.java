...
public class FaceServer {
     ...
     * @param registerFaceEngine 用于{@link FaceEngine#extractFaceFeature(byte[], int, int, int, FaceInfo, FaceFeature)}注册人脸到本地数据库
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FacePreviewInfo faceInfo, String name,
        ...
        synchronized (registerFaceEngine) {
            code = registerFaceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfo.getFaceInfoRgb(), faceFeature);
        }
    ...
    private void registerFaceFeatureInfoListFromDb(FaceEngine faceEngine, List<FaceEntity> faceEntityList) {
        ...
        if (faceEngine != null) {
            ...
            Log.i(TAG, "registerFaceFeature:" + res + ", faceList:" + faceFeatureInfoList.size());
        }
    }
    ...
    public CompareResult searchFaceFeature(FaceFeature faceFeature, FaceEngine faceEngine) {
        ...
        try {
            ...
            //根据实际项目需要修改topN的值
            List<SearchResult> searchResultList = faceEngine.searchFaceFeatureTopN(faceFeature, 1);
