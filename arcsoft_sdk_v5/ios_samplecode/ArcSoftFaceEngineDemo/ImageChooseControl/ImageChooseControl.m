...
- (void)buildViews{
    ...
    [SelfBtnStyleUtil setupDefaultButtonStyle:_imgBtn title:_pickerTitle];
...
//    _markLabel.layer.cornerRadius = 10.0;
...
- (void)clickClearBtn{
    ...
    if ([self.delegate respondsToSelector:@selector(imageChooseControl:didClearImage:identify:)]) {
        [self.delegate imageChooseControl:self didClearImage:nil identify:self.identify];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary*)info {
    ...
    _image = image;
    if ([self.delegate respondsToSelector:@selector(imageChooseControl:didChooseFinished:identify:)]) {
        [self.delegate imageChooseControl:self didChooseFinished:image identify:self.identify];
    }
