 ...
 * @returns 注册结果
 */
export const registerFaceFromUrl = async (userId: string, imageUrl: string, maxRetries: number = 2): Promise<{ success: boolean; msg: string; userId?: string; featureBase64?: string }> => {
    let attempt = 0;
    let lastErrorMsg = '';
    ...
    return { success: false, msg: `注册失败(重试${maxRetries}次后): ${lastErrorMsg}` };
};
